...
"{ output         |       | resulting output file}";

int main(int argc, char** argv)
{
    //const string OUTNAME = "v3_TIME_yolo_detect.mp4";
    ...
    String outName = parser.get<String>("output");
    ...
    outputVideo.open(outName.c_str()/*OUTNAME*/, CV_FOURCC('M','J','P','G'), cap.get(CV_CAP_PROP_FPS), S, true);
    ...
    for(;;)
    {
        ...
        outputVideo << frame;
        imshow("TIME Magazine Detections", frame);
        if (waitKey(1) >= 0) 
        {
            break;
        }
